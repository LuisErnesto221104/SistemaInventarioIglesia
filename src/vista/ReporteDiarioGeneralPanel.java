...
/**
 * Panel para mostrar el reporte diario general (adultos e infantiles)
 */
public class ReporteDiarioGeneralPanel extends JPanel {
     ...
     */
    public ReporteDiarioGeneralPanel(MenuPrincipal menuPrincipal) {
    ...
    private void inicializarComponentes() {
        ...
        lblTitulo = new JLabel("Reporte Diario General");
        ...
        String[] columnas = {"Tipo Socio", "No. Socio", "Nombre", "Apellido(s)", 
        ...
        modeloTabla = new DefaultTableModel(columnas, 0) {
            ...
            public Class<?> getColumnClass(int columnIndex) {
                if (columnIndex >= 4) return Double.class; // Columnas numéricas
                return String.class;
            }
        };
        ...
        tablaReporte.setAutoResizeMode(JTable.AUTO_RESIZE_OFF); // Permitir scroll horizontal
        ...
        for (int i = 4; i < tablaReporte.getColumnCount(); i++) {
            tablaReporte.getColumnModel().getColumn(i).setCellRenderer(moneyRenderer);
        }
        
        // Ajustar anchos de columna
        tablaReporte.getColumnModel().getColumn(0).setPreferredWidth(80); // Tipo Socio
        tablaReporte.getColumnModel().getColumn(1).setPreferredWidth(60); // No. Socio
        tablaReporte.getColumnModel().getColumn(2).setPreferredWidth(120); // Nombre
        tablaReporte.getColumnModel().getColumn(3).setPreferredWidth(150); // Apellido(s)
        
        // Configurar anchos para columnas monetarias
        for (int i = 4; i < tablaReporte.getColumnCount(); i++) {
            tablaReporte.getColumnModel().getColumn(i).setPreferredWidth(100);
        }
    ...
    private void generarReporte() {
        ...
        try {
            // Consulta SQL para obtener los movimientos de socios adultos e infantiles en la fecha seleccionada
            String consulta = "SELECT ms.NoSocio, ms.TipoSocio, " +
                         "CASE WHEN ms.TipoSocio = 'ADULTO' THEN s.Nombres ELSE si.Nombres END AS Nombres, " +
                         "CASE WHEN ms.TipoSocio = 'ADULTO' THEN s.Apellidos ELSE si.Apellidos END AS Apellidos, " +
                         "ms.AporIngresos, ms.PresIngresos, ms.Intereses, ms.AhoIngresos, " +
                         "ms.SaldoBanco, ms.AporEgresos, ms.PresEgresos, ms.RetInteres, ms.AhoEgresos, ms.RetBanco " +
                         "FROM MovimientosSocio ms " +
                         "LEFT JOIN Socios s ON (ms.NoSocio = s.NoSocio AND ms.TipoSocio = 'ADULTO') " +
                         "LEFT JOIN SociosInfa si ON (ms.NoSocio = si.NoSocio AND (ms.TipoSocio = 'INFANTIL' OR ms.TipoSocio = 'INFANTE')) " +
                         "WHERE ms.Fecha = ? " +
                         "ORDER BY ms.TipoSocio, ms.NoSocio";
            ...
            lblTitulo.setText("Reporte Diario General - " + fechaFormateada);
    ...
    private void imprimirReporte() {
        try {
            ...
            MessageFormat header = new MessageFormat("Reporte Diario General - " + fechaFormateada);
